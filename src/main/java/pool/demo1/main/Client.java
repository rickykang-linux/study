package pool.demo1.main;

import pool.demo1.ConnectionPoolManager;
import pool.demo1.IConnectionPool;
import pool.demo1.ThreadConnection;
...
// 设置优先级，先让初始化执行，模拟 线程�?先启�?
// 这里仅仅表面控制了，因为即使t 线程先启动，也不能保证pool 初始化完成，为了�?��模拟，这里先这样写了
        t1.setPriority(10);
        t2.setPriority(10);
        t3.setPriority(10);
        t1.start();
