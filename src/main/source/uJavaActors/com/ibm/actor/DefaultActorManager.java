...
public class DefaultActorManager extends Utils implements ActorManager {
	...
	/**
	 * Detach an akka.actor.
	 */
	@Override
	public void detachActor(Actor actor) {
		if (((AbstractActor) actor).getManager() != this) {
			throw new IllegalStateException("akka.actor not owned by this manager");
		}
	...
	public int send(Message message, Actor from, String category) {
		...
		// find an akka.actor with lowest message count
	...
	/**
	 * Suspend an akka.actor until it has a read message.
	 * 
	 * @param actor
	 *            receiving akka.actor
	 */
	...
	public class ActorRunnable implements Runnable {
		...
		public void run() {
			...
			while (running) {
				try {
					if (!procesNextActor()) {
						// logger.trace("procesNextActor waiting on akka.actor");
						// sleep(delay * 1000);
						synchronized (actors) {
	...
	/**
	 * Create an akka.actor and associate it with this manager.
	 * 
	 * @param clazz
	 *            the akka.actor class
	 * @param the
	 *            akka.actor name; must be unique
	 */
	@Override
	...
	/**
	 * Create an akka.actor and associate it with this manager then start it
	 * 
	 * @param clazz
	 *            the akka.actor class
	 * @param the
	...
	/**
	 * Start an akka.actor. Must have been created by this manager.
	 * 
	 * @param actor
	 *            the akka.actor
	 */
