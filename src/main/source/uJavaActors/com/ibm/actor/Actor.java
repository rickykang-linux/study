 ...
 * to a category and have them processed by an arbitrary akka.actor supporting that
 ...
 * 
 * Since an akka.actor consumes a thread while processing a message and threads are a
 * constrained resource, the akka.actor should keep message processing as brief as
 ...
 * 
 * The run() method is called once when the akka.actor registers with a manager. The
...
public interface Actor extends Runnable {
	...
	/** Get an akka.actor's name. Must not be null once assigned to a manager */
	String getName();
	...
	/**
	 * Get the akka.actor's category. The category is used to group actors as send
	 * targets.
	 */
	String getCategory();

	/** Set the akka.actor's category. Do not set to null (use DEFAULT_CATEGORY). */
	void setCategory(String category);

	/** Process the next incoming message. */
	boolean receive();

	/** Test if this akka.actor will receive a send with a specified subject. */
	...
	/**
	 * Tell this akka.actor it is active. Messages may be received before this call
	 * but they should be ignored.
	 **/
	void activate();

	/**
	 * Tell this akka.actor it is no longer active. Messages may be received after
	...
	/**
	 * End reception of new messages. Typically used before removing an akka.actor.
